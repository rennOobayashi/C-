...
namespace Week09Homework
{
    public partial class FormManager : Form
    {
                ...
                using (var fs = new FileStream(DepartmentFullFileName, FileMode.Open))
                {
                    using (var sr = new StreamReader(fs))
                    {
                        ...
                        while (false == sr.EndOfStream)
                        {
                            ...
                            if (dept != null && deptInd < departments.Length)
                            {
                                departments[deptInd++] = dept;
        ...
        private void btnRegisterDepartment_Click(object sender, EventArgs e)
        {
            ...
            //file append
            using (var fs = new FileStream(DepartmentFullFileName, FileMode.Append)) //알아서 닫음
            {
                using (var sw = new StreamWriter(fs))
                {
                    sw.WriteLine(dept.Recode);
                }
            }
        }
        ...
        private void btnRemoveDepartment_Click(object sender, EventArgs e)
        {
            ...
            if (lbxDepartment.SelectedItem is Department)
            {
                ...
                try
                {
                    using (var fs = new FileStream(DepartmentFullFileName, FileMode.Create)) //있으면 덮어쓰기
                    {
                        using (var sw = new StreamWriter(fs))
                        {
                ...
                {
                    Console.WriteLine("배열 원소 중간에 null\r\n(" + ex.Message + ")");
                }
                catch (Exception ex)
                {
