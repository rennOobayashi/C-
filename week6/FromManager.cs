...
namespace Week04Homework
{
    public partial class FormManager: Form
    {
        ...
        private void btnTestSearchStudent_Click(object sender, EventArgs e)
        {
            ...
            if (grade != null)
            {
                //grade의 Scores가 tbxtestScores보다 많을 수도 있고 그 반대가 발생할 수도 있음
        ...
        private void btnTestRegScore_Click(object sender, EventArgs e)
        {
            ...
            //학생 성적 입력한 항목 중 중간이 비어있으면 안됨
            for (int i = 1; i < tbxTestScores.Length; ++i)
            {
                if (false == string.IsNullOrEmpty(tbxTestScores[i].Text) && string.IsNullOrEmpty(tbxTestScores[i - 1].Text))
                {
                    MessageBox.Show($"과목{i + 1}의 점수 값이 비어있습니다.");
            ...
            for (int i = 0; i < tbxTestScores.Length; ++i)
            {
                ...
                {
                    MessageBox.Show($"과목{i + 1}의 점수 값이 잘못 입력되었습니다.");
            ...
            if (avg < 0)
            {
                ...
                //굳이 focus할 필요는 없을 듯
                tbxTestScores[0].Focus();
                return;
            }

            //소수점 한자리까지 %.1f
            //%02d는 D2
            lblTestAverage.Text = avg.ToString("F1");
        }
    }
}
